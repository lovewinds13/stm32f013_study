...
void timer4_cap_init(u16 timer_arr, u16 timer_psc)
{
	...
	gp_init.GPIO_Pin  	= ((uint16_t)0x0100); 
	gp_init.GPIO_Mode 	= GPIO_Mode_IPD; 
	GPIO_Init(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00)), &gp_init);

	gp_init.GPIO_Pin  	= ((uint16_t)0x0200);     
	gp_init.GPIO_Mode 	= GPIO_Mode_Out_PP;     
	gp_init.GPIO_Speed 	= GPIO_Speed_2MHz;     
	GPIO_Init(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00)), &gp_init);

	
	tim_base_init.TIM_Period 		= timer_arr; 
	tim_base_init.TIM_Prescaler 	= timer_psc; 	
	tim_base_init.TIM_ClockDivision = ((uint16_t)0x0000); 
	tim_base_init.TIM_CounterMode 	= ((uint16_t)0x0000);  
	TIM_TimeBaseInit(((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0800)), &tim_base_init); 

	
	tim_ic_init.TIM_Channel 	= ((uint16_t)0x0008); 
	tim_ic_init.TIM_ICPolarity 	= ((uint16_t)0x0000);	
	tim_ic_init.TIM_ICSelection = ((uint16_t)0x0001);
	tim_ic_init.TIM_ICPrescaler = ((uint16_t)0x0000);	 
	tim_ic_init.TIM_ICFilter 	= 0x00;
	TIM_ICInit(((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0800)), &tim_ic_init);

	
	nvic_init_config.NVIC_IRQChannel 					= TIM4_IRQn;  
	nvic_init_config.NVIC_IRQChannelPreemptionPriority 	= 3;  
	nvic_init_config.NVIC_IRQChannelSubPriority 		= 3;  
	nvic_init_config.NVIC_IRQChannelCmd 				= ENABLE; 
	NVIC_Init(&nvic_init_config);  
	
	TIM_ITConfig(((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0800)), ((uint16_t)0x0001)|((uint16_t)0x0008), ENABLE);
...
{
	GPIO_SetBits(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00)), ((uint16_t)0x0200));	
	delay_us(15);
	GPIO_ResetBits(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00)), ((uint16_t)0x0200));
	...
	if (g_cap_state & 0X80)	
	{
		...
		g_cap_distance = g_cap_distance * 170 / 1000;	
		g_cap_state = 0X00;
	}
}
...
void TIM4_IRQHandler(void)
{
	...
	sta_val = ((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0800))->SR;
	if ((g_cap_state & 0X80) == 0X00)	
	{
		if (sta_val & 0X01)	
		{
			if (g_cap_state & 0X40)	
			{
				if ((g_cap_state & 0X3f) == 0X3f)	
				{
					g_cap_state |= 0X80;	
					g_cap_val = 0Xffff;	
				}
		...
		if (sta_val & 0X08) 
		{
			...
			{
				g_cap_state |= 0X80;	
				g_cap_val = ((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0800))->CCR3;	
