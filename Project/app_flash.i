...
void W25_CircleWriteRead(uint8_t _ucFlashSize,uint32_t _ulAddr, uint8_t *ucpWBuf, uint8_t *ucpRBuf, uint16_t _usLen)
{
    ...
    for (uint16_t i = 0; i < usPageSum; i++)
    {
        ...
        Flash_WriteSomeBytes(ucpWBuf, _ulAddr, _usLen);	
		...
		printf("\r\n");
		Sys_delay_ms(500);
        Flash_ReadSomeBytes(ucpRBuf, _ulAddr, _usLen);	
        printf("Read Page %d -> \r\n",i);
		for(usCnt = 0; usCnt < _usLen; usCnt++)
		{
			 printf("0x%02X ",ucpRBuf[usCnt]);	
		...
		for(usCnt = 0; usCnt < _usLen ; usCnt++)
		{
			if(ucpWBuf[usCnt] != ucpRBuf[usCnt])	
			{
...
void W25_WriteReadBytes(void)
{
	...
	Flash_WriteSomeBytes((uint8_t *)ucWriteBuf, (0x100), 10);
	...
	Sys_delay_ms(500);
	Flash_ReadSomeBytes(ucReadBuf, (0x100), 10);	
	printf("Read Data is ---->\r\n");
	for (uint16_t i = 0; i < 10; i++)
	{
...
void W25_EraseTest(void)
{
	...
	Flash_EraseChip();	
	Sys_delay_ms(500);	
	Flash_ReadSomeBytes(ucaTmpBuf, 0x00, 0x100);	
